...
import { materials, lessons } from "@/lib/mock-data";
    ...
    >
      <div className="rounded-2xl border border-border bg-card shadow-soft overflow-hidden">
                  ...
                  <td className="py-3 px-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="md:hidden text-xs text-muted-foreground truncate">{m.lesson}</div>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 px-5 hidden md:table-cell text-muted-foreground">{m.lesson}</td>
                  <td className="py-3 px-5 hidden lg:table-cell text-muted-foreground">{m.size}</td>
                  <td className="py-3 px-5 hidden lg:table-cell text-muted-foreground">{m.uploadedBy}</td>
                  <td className="py-3 px-5 hidden sm:table-cell text-muted-foreground">{m.uploadedAt}</td>
                  <td className="py-3 px-5">
                    <div className="flex justify-end items-center gap-1">
                      ...
                      <button className="grid h-8 w-8 place-items-center rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition" title="Delete"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-border text-xs text-muted-foreground">
          <div>Showing 1–{materials.length} of {materials.length} files</div>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-foreground/40 backdrop-blur-sm p-4 animate-in fade-in" onClick={onClose}>
      <div className="w-full max-w-lg rounded-3xl bg-card shadow-lift border border-border p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-1">
          ...
          <button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg text-muted-foreground hover:bg-muted hover:text-foreground transition"><X className="h-4 w-4" /></button>
        </div>
        ...
        <label
          ...
          onDrop={(e) => { e.preventDefault(); setDragOver(false); const f = e.dataTransfer.files?.[0]; if (f) fakeUpload(f.name); }}
        ...
        >
          <input type="file" accept="application/pdf" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) fakeUpload(f.name); }} />
        ...
        <div className="mt-5 space-y-3">
          <Input label="Title" placeholder="e.g. Lecture 5 — Series & Convergence" />
          <div>
            <label className="text-sm font-medium">Lesson</label>
            <select className="mt-1.5 w-full rounded-xl border border-input bg-card px-3 py-2.5 text-sm outline-none focus:border-primary">
              {lessons.map(l => <option key={l.id}>{l.title}</option>)}
            </select>
          </div>
          <Input label="Description (optional)" placeholder="A short note about this document" />
        </div>

        <div className="mt-6 flex gap-2 justify-end">
          <button onClick={onClose} className="rounded-xl border border-border bg-card px-4 py-2 text-sm font-semibold hover:bg-muted transition">Cancel</button>
          <button onClick={onClose} className="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition">Upload</button>
        </div>
      </div>
    </div>
  );
